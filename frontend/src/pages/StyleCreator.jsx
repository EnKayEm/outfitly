...
export default function StyleCreator() {
  ...
  // --- NOWE STANY DO PODMIAN (SWAP) ---
  const [isSwapModalOpen, setIsSwapModalOpen] = useState(false);
  const [itemToSwap, setItemToSwap] = useState(null); // ID ubrania, które aktualnie wymieniamy
      ...
      <div className="max-w-5xl mx-auto animate-in slide-in-from-bottom-4 duration-500">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4 border-b border-slate-200 pb-6">
          ...
          <div className="flex gap-3 w-full md:w-auto">
            <button onClick={(e) => handleMockGenerate(e)} className="flex-1 md:flex-none px-5 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-xl font-medium hover:bg-slate-50 hover:text-purple-600 hover:border-purple-200 transition-all flex items-center justify-center gap-2 shadow-sm">
              ...
              {outfitClothes.map(item => (
                <ClothingCard key={item.id} item={item} onClick={() => console.log('Kliknięto podgląd ubrania')}>
                  <button 
                    ...
                    className="group/btn absolute top-3 right-3 bg-white/90 backdrop-blur-sm p-2 rounded-lg shadow-sm border border-slate-200 text-slate-600 opacity-0 group-hover:opacity-100 hover:text-purple-600 hover:border-purple-300 transition-all"
                  >
        ...
        <div className="relative z-10">
          ...
          <form onSubmit={handleGenerate} className="flex flex-col gap-4">
            ...
            <div className="mt-6 flex flex-col sm:flex-row gap-3">
              ...
              <button type="button" onClick={handleMockGenerate} disabled={clothes.length === 0} className="sm:w-1/3 py-4 bg-slate-800 text-white text-sm font-bold rounded-2xl hover:bg-slate-900 transition-all disabled:opacity-50 flex justify-center items-center gap-2" title="Losuje ubrania z bazy (Przydatne przy limitach)">
